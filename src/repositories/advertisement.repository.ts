...
import { ICurrency, IExchange, IPrices } from "../interfaces";
import { Currency, Statistic } from "../models";
import { axiosService } from "../services";
import { currencyRepository } from "./currency.repository";

class AdvertisementRepository {
  public async convertCurrency(currency: IExchange): Promise<IPrices> {
    try {
      ...
      switch (currency.base_ccy) {
        case "UAH":
          return {
            USD: Number(
              (Number(currency.price) / Number(currencies.usd_ccy)).toFixed(5),
            ),
            EUR: Number(
              (Number(currency.price) / Number(currencies.eur_ccy)).toFixed(5),
            ),
        ...
        case "USD":
          return {
            ...
            EUR: Number(
              (
                (Number(currency.price) * Number(currencies.eur_ccy)) /
                Number(currencies.usd_ccy)
              ).toFixed(5),
            ),
            UAH: Number(
              (Number(currency.price) * Number(currencies.usd_ccy)).toFixed(5),
        ...
        case "EUR":
          return {
            USD: Number(
              (
                (Number(currency.price) * Number(currencies.usd_ccy)) /
                Number(currencies.eur_ccy)
              ).toFixed(5),
            ),
